"""Ingest Claude Code session JSONL files into the SQLite ``traces`` table.

Walks ``data/raw/claude_code/*.jsonl`` (override with ``--raw-dir``) and runs
each file through :func:`trace_marketplace.ingest.pipeline.ingest_file`. Same
``IngestResult`` reporting and idempotent upsert semantics as
``scripts/ingest_atif.py``.

Run:
    uv run python scripts/ingest_claude_code.py
    uv run python scripts/ingest_claude_code.py --files path/to/one.jsonl
"""
...
DEFAULT_RAW_DIR = PROJECT_ROOT / "data" / "raw" / "claude_code"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=f"Directory of *.jsonl files to ingest (default: {DEFAULT_RAW_DIR}).",
    )
    parser.add_argument(
        "--files",
        nargs="+",
    ...
    print(f"Ingesting {len(paths)} Claude Code file(s) into {args.db}")
    ...
    with connect(args.db) as conn:
        ...
        for path in paths:
            ...
            print(
                f"  {status} {path.name:50s} -> {result.trace_id}  [{result.source_format}]"
            )
            if result.validated:
                validated_count += 1
